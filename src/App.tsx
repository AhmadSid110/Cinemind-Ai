...
const App: React.FC = () => {
  ...
  // flag to avoid "local → cloud" before we've decided cloud vs local on login
  const [cloudLoadedForUser, setCloudLoadedForUser] = useState(false);
  ...
  // ----- FIREBASE AUTH SUBSCRIPTION (ONLINE-FIRST LOGIC) -----
  useEffect(() => {
    const unsub = subscribeToAuthChanges(async (user) => {
      setCurrentUser(user);
      setCloudLoadedForUser(false);

      if (user) {
        try {
          setIsSyncingCloud(true);

          // 1) Try to load CLOUD data
          const cloud = await loadUserData(user.uid);

          if (cloud) {
            // ✅ Cloud doc exists → treat cloud as the source of truth
            const favoritesFromCloud = cloud.favorites ?? [];
            const watchlistFromCloud = cloud.watchlist ?? [];
            const tmdbFromCloud = cloud.tmdbKey ?? state.tmdbKey ?? '';
            const geminiFromCloud = cloud.geminiKey ?? state.geminiKey ?? '';

            setState((prev) => ({
              ...prev,
              favorites: favoritesFromCloud,
              watchlist: watchlistFromCloud,
              tmdbKey: tmdbFromCloud,
              geminiKey: geminiFromCloud,
            }));

            // Sync localStorage to cloud values
            localStorage.setItem('favorites', JSON.stringify(favoritesFromCloud));
            localStorage.setItem('watchlist', JSON.stringify(watchlistFromCloud));
            if (tmdbFromCloud) localStorage.setItem('tmdb_key', tmdbFromCloud);
            if (geminiFromCloud) localStorage.setItem('gemini_key', geminiFromCloud);
          } else {
            // ❌ No cloud doc yet → push LOCAL data up ONCE
            const localFavorites = JSON.parse(localStorage.getItem('favorites') || '[]');
            const localWatchlist = JSON.parse(localStorage.getItem('watchlist') || '[]');
            const localTmdb = localStorage.getItem('tmdb_key') || '';
            const localGemini = localStorage.getItem('gemini_key') || '';
            ...
            // also ensure state matches what we just pushed
            setState((prev) => ({
              ...prev,
              favorites: localFavorites,
              watchlist: localWatchlist,
              tmdbKey: localTmdb || prev.tmdbKey,
              geminiKey: localGemini || prev.geminiKey,
            }));
          }
        } catch (err) {
          console.error('Error handling cloud sync on login:', err);
        } finally {
          setIsSyncingCloud(false);
          setCloudLoadedForUser(true); // ✅ from now on, regular syncing is allowed
        }
      } else {
        // user logged out
        setCloudLoadedForUser(false);
    ...
    return () => unsub();
    // we intentionally keep deps empty to avoid re-subscribing on state changes
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // ----- CONTINUOUS SYNC: LOCAL CHANGES → CLOUD (AFTER CLOUD LOADED) -----
  useEffect(() => {
    const sync = async () => {
      if (!currentUser || !cloudLoadedForUser) return;
    ...
    sync();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentUser, cloudLoadedForUser, state.favorites, state.watchlist, state.tmdbKey, state.geminiKey]);
        ...
        onSave={async (key, geminiKey) => {
          ...
          if (currentUser) {
            try {
              ...
              console.error('Error saving keys to Firestore from settings:', err);
            }
          }
        }}
      />
