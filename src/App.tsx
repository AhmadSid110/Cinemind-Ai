...
// (If you haven't moved Auth/Home logic to hooks yet, we keep them inline below)
...
const App: React.FC = () => {
  ...
  const [state, setState] = useState<AppState>({
    view: 'trending',
    // searchQuery is now managed by the hook, but we keep it here for header input if needed, 
    // OR we let the hook drive. For this update, we sync them.
    tmdbKey: localStorage.getItem('tmdb_key') || '',
    
    // CHANGED: Initialize Array of Keys
    geminiKeys: (() => {
        try {
    ...
    searchResults: [], // Managed by hook, but kept for view switching logic compatibility
    selectedItem: null,
    selectedPerson: null,
    isLoading: false,
    error: null,
  ...
  // ---------- HOME DATA (Trending) ----------
  // If you haven't made useHomeFeed.ts yet, we keep this state here:
  ...
  const searchHook = useMediaSearch({
    ...
    geminiKeys: state.geminiKeys || [], // Pass the array!
    trendingMovies,
    trendingTv
  });
  ...
  useEffect(() => {
    if (searchHook.results.length > 0 || searchHook.explanation) {
       // When hook finds stuff, switch view
       setState(prev => ({ 
           ...prev, 
           view: 'search',
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans pb-24 selection:bg-cyan-500/30">
      ...
      <SettingsModal 
        ...
        currentGeminiKeys={state.geminiKeys} // Pass Array
        onSave={(key, keys) => {
